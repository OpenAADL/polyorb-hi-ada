...
procedure Headers_PO_HI is
   ...
   "------------------------------------------------------------------------------" & ASCII.LF &
   "--                                                                          --" & ASCII.LF &
   "--                          PolyORB HI COMPONENTS                           --" & ASCII.LF &
   "--                                                                          --" & ASCII.LF &
   "@UNIT_NAME@" &
   "--                                                                          --" & ASCII.LF &
   "@COPYRIGHT@" &
   "--                                                                          --" & ASCII.LF &
   "-- PolyORB HI is free software; you  can  redistribute  it and/or modify it --" & ASCII.LF &
   ...
   "-- PolyORB HI is distributed  in the hope that it will be useful, but       --" & ASCII.LF &
   ...
   "-- GNU General Public  License  distributed with PolyORB HI; see file       --" & ASCII.LF &
   "-- COPYING. If not, write  to the Free  Software Foundation, 51 Franklin    --" & ASCII.LF &
   "-- Street, Fifth Floor, Boston, MA 02111-1301, USA.                         --" & ASCII.LF &
   "--                                                                          --" & ASCII.LF &
   ...
   procedure Update_Header (Filename : String) is
      ...
      Unit_Name_Matcher : constant Pattern_Matcher :=
        Compile ("^(private\s+)?(procedure|function|project|"
